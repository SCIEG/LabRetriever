...
dg = c(0.3) #Put the DO probs here
...
d19 = read.table("/Users/sb7689/Dropbox/KKKN/Statistics_DO programs/Frequency tables/D19_B.count.csv",head=T,sep=",")
nd19 = d19[,race]	# allele counts
names(nd19) = d19[,2]		# allele names
ssd19 = c("15")
calld19 = list(list(c("15"),c("14.2")))
...
totLR<-LRd8*LRd21*LRd3*LRtho*LRd13*LRd16*LRd19*LRvwa*LRtpo*LRd5*LRfga

#write the per-locus LRs to a file:
locus_LRs<-rbind(headers,d8_LRs,d21_LRs,d3_LRs,tho_LRs,d13_LRs,d16_LRs,d19_LRs,vwa_LRs,tpo_LRs,d5_LRs,fga_LRs)
locus_totals<-cbind("Totals",totnmr,totdnm,totLR)

write.table(locus_LRs,"/Users/sb7689/Desktop/Lab Retriever Temp testing/Wood Identifiler_test.txt", sep="\t",col.names=FALSE, row.names=FALSE, append=TRUE)

write.table(locus_totals,"/Users/sb7689/Desktop/Lab Retriever Temp testing/Wood Identifiler_test.txt", sep="\t",col.names=FALSE, row.names=FALSE, append=TRUE)

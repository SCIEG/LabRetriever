...
dg = c(0.02) #Put the DO probs here
...
calld8 = list(list(c("11"),c("12","15"))) # the first list has the minor alleles that are not from an "assumed contributor," and can be any number of alleles, dependent on the R functions for which it will serve as input data.  Any alleles that 1) ARE from an assumed contributor, OR were 2) in a stutter position, AND 3) above the detection threshold, AND 4) below the stutter threshold would go where the word "NULL" is.
...
ssd21 = c("30","32.2")
calld21 = list(list(c("30","32.2"),c("29","31.2")))
...
sstho = c("7")
calltho = list(list(c(NULL),c("7","9")))  
...
ssd13 = c("11","12")
calld13 = list(list(c(NULL),c("11","12")))
...
ssd16 = c("9","11")
calld16 = list(list(c(NULL),c("9","10","11","12")))
...
ssvwa = c("15","18")
callvwa = list(list(c("18"),c("15","16","17")))
...
sstpo = c("11")
calltpo = list(list(c(NULL),c("9","11")))
...
ssd18 = c("13","21")
calld18 = list(list(c("13","21"),c("15","22")))
...
write.table(locus_LRs,"/Volumes/Inman Drive 2 1Tb/Dropbox/KKKN/Lab Retriever/Lab Retriever testing/Test 8/Dozier 15C 1Unk R results.txt", sep="\t",col.names=FALSE, row.names=FALSE, append=TRUE)

write.table(locus_totals,"/Volumes/Inman Drive 2 1Tb/Dropbox/KKKN/Lab Retriever/Lab Retriever testing/Test 8/Dozier 15C 1Unk R results.txt", sep="\t",col.names=FALSE, row.names=FALSE, append=TRUE)
